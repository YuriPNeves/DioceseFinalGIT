        ...
        <?php foreach ($cursos as $curso): ?>
            <ul class="collection">
                <li class="collection-item avatar">
                    ...
                    <blockquote><?php echo h('Valor: '.$curso['Curso']['valor'].' R$'); ?></blockquote>
                    </p>
                    <div class="secondary-content">
                        <?php
                        echo $this->Html->link($this->Html->tag('i', 'visibility', array('class' => 'material-icons white-text ')), 
						array('controller' => 'cursos', 'action' => 'view', $curso['Curso']['id']), array('escape' => FALSE, 
						'class' => 'btn-floating btn-small waves-effect waves-light purple tooltipped',
                        'data-position'=>'top', 'data-delay'=>'50', 'data-tooltip'=>"Visualizar ".$curso['Curso']['nome']));
                        ?>
                        <?php
    ...
    <div class="col m12 l12 s12">
        <p>
            ...
            echo $this->Paginator->counter(array(
                'format' => __('Página {:page} de {:pages}, mostrando {:current} Cursos de um total de {:count}, começando em {:start}, terminando em {:end}')
            ));
            ?>		</p>
        <ul class="pagination">
            <?php
